...
export class SDOOwnerWriter {
    ...
    private static cborCache = new Map<string, string>();
    static cborEncodeText(text: string) {
        if (this.cborCache.has(text)) {
            return this.cborCache.get(text)!
        }
        const encoded = cbor.encode({text}).toString('hex')
        this.cborCache.set(text, encoded)
        return encoded
    }
    ...
    public getLockingSubScript() {
        // only get the subscript after OP_CODESEPARATOR

        const UIDHex = SDOOwnerWriter.cborEncodeText(this.UID)
        const keyHex = SDOOwnerWriter.cborEncodeText(this.key)
        ...
        const lockingScript = 
            ...
            bsv.Script.fromASM("OP_ENDIF").toHex() +
            SDOOwnerWriter.lockingScriptTemplate.substring(0, SDOOwnerWriter.ownerAddrPos) + 
            SDOOwnerWriter.getAddressHex(this.ownerAddress) + 
            SDOOwnerWriter.lockingScriptTemplate.substring(SDOOwnerWriter.ownerAddrPos + SDOOwnerWriter.ADDRLENINBYTES*2, SDOOwnerWriter.writerAddrPos) + 
            SDOOwnerWriter.getAddressHex(this.writerAddress) + 
            SDOOwnerWriter.lockingScriptTemplate.substring(SDOOwnerWriter.writerAddrPos + SDOOwnerWriter.ADDRLENINBYTES*2)
        return lockingScript
    }
    ...
    public getWriterUpdateScriptPair(newValueString: string, privateKey: bsv.PrivateKey, publicKey: bsv.PublicKey) {
        ...
        const prevTxId = this.txId
        const prevOutIndex = this.outIndex
        const prevOutLockingSubScript = this.getLockingSubScript()
        const prevOutSatoshis = this.satoshis
